function [res, convergence,rmse] = cgL1ESPIRiT_mod(kData, x0, FT, MapOp, nIterCG, XOP, lambda, alpha,nIterSplit,im_ref)
%
...
for n=1:nIterSplit;
    ...
    % figure(100), imshow3(abs(res),[]), drawnow;
     ...
     convergence(n)= norm(obj1(:));
    im_out = crop(abs(sos(weight(MapOp,res))),256,256);    
    rmse(n) = RMSE(mat2gray(im_ref),mat2gray((im_out)));
    disp(sprintf('Iteration: %d, consistency: %f RMSE: %f',n,norm(obj1(:)),rmse(n)));
end
figure,  plot([1:size(convergence,2)],convergence,'LineWidth',2);
