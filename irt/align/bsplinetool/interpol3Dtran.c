/*******************************************************************************
        ...
        Calculate the transpose of interpolation matrices
...
{ /* begin InterpolatedValue3DTranMirr */
	...
	switch (SplineDegree) {
		...
		case 6L:
			...
			yWeight[5] = 1.0 - yWeight[0] - yWeight[1] 
				- yWeight[2] - yWeight[3]
				- yWeight[4] - yWeight[6];
			/* z */
			w = z - (double)zIndex[3];
			...
			zWeight[5] = 1.0 - zWeight[0] - zWeight[1] 
				- zWeight[2] - zWeight[3]
				- zWeight[4] - zWeight[6];
			break;
		case 7L:
		...
		case 8L:
			...
			xWeight[6] = 1.0 - xWeight[0] - xWeight[1] 
				- xWeight[2] - xWeight[3] - xWeight[4] 
				- xWeight[5] - xWeight[7] - xWeight[8];
			/* y */
			w = y - (double)yIndex[4];
			yWeight[0] = 1.0 / 2.0 - w;
			yWeight[0] *= yWeight[0];
			...
			yWeight[2] = (82903.0 / 1792.0 + w * (-4177.0 / 32.0 + w
				* (2275.0 / 16.0 + w * (-487.0 / 8.0 
				+ w * (-85.0 / 8.0 + w * (41.0 / 2.0 
				+ w * (-5.0 + w * (-2.0 + w)))))))) / 1440.0;
		...
		case 9L:
			...
			xWeight[2] = (3652.0 / 9.0 - w * (2023.0 / 2.0 
				+ w * (-952.0 + w * (938.0 / 3.0 
				+ w * (112.0 + w * (-119.0 + w * (56.0 / 3.0 + w
				* (14.0 + w * (-7.0 + w))))))))) / 10080.0;
			xWeight[3] = (44117.0 / 42.0 + w * (-2427.0 / 2.0 
				+ w * (66.0 + w * (434.0 + w * (-129.0 
				+ w * (-69.0 + w * (34.0 + w * (6.0 + w
				* (-6.0 + w))))))))) / 4320.0;
			w2 = w * w;
			...
			xWeight[6] = (3652.0 / 21.0 - w * (-867.0 / 2.0 
				+ w * (-408.0 + w * (-134.0 + w * (48.0 
				+ w * (51.0 + w * (-4.0 + w) * (-1.0 + w)
				* (2.0 + w))))))) / 4320.0;
			...
			yWeight[2] = (3652.0 / 9.0 - w * (2023.0 / 2.0 
				+ w * (-952.0 + w * (938.0 / 3.0 + w * (112.0 
				+ w * (-119.0 + w * (56.0 / 3.0 + w
				* (14.0 + w * (-7.0 + w))))))))) / 10080.0;
	...
	for (j = 0L; j <= SplineDegree; j++) {
		for (l = 0L; l <= SplineDegree; l++) {
			p = Bout + (ptrdiff_t)(yIndex[l] * Width 
				+ zIndex[j] * Width * Height);

			for (i = 0L; i <= SplineDegree; i++) {
				p[xIndex[i]] += zWeight[j] * yWeight[l] 
					* xWeight[i] * Bcoeff;
			}
		}
	}
...
} /* end InterpolatedValue3DTranMirr */
...
{ /* begin InterpolatedGradX3DTranMirr */
...
} /* end InterpolatedGradX3DTranMirr */
...
{ /* begin InterpolatedGradY3DTranMirr */
	...
	for (j = 0L; j <= SplineDegree; j++) {
		for (l = 0L; l <= SplineDegree; l++) {
			...
			for (i = 0L; i <= SplineDegree; i++) {
				p[xIndex[i]] += zWeight[j] * gradyWeight[l] 
					* xWeight[i] * Bcoeff;
			}
		}
	}
...
} /* end InterpolatedGradY3DTranMirr */
...
{ /* begin InterpolatedGradZ3DTranMirr */
	...
	for (j = 0L; j <= SplineDegree; j++) {
		for (l = 0L; l <= SplineDegree; l++) {
			...
			for (i = 0L; i <= SplineDegree; i++) {
				p[xIndex[i]] += gradzWeight[j] * yWeight[l] 
					* xWeight[i] * Bcoeff;
			}
		}
	}
...
} /* end InterpolatedGradZ3DTranMirr */
...
{ /* begin InterpolatedValue3DTranZero */
...
} /* end InterpolatedValue3DTranZero */
...
{ /* begin InterpolatedGradX3DTranZero */
           ...
           if ((zIndex[j] >= 0) && (zIndex[j] < Slice)) {
              for (l = 0L; l <= SplineDegree; l++) {
				...
				+ zIndex[j] * Width * Height); 
                    for (i = 0L; i <= SplineDegree; i++) {
                       if ((xIndex[i] >= 0) && (xIndex[i] < Width)) {
                         p[xIndex[i]] += zWeight[j]*yWeight[l]*gradxWeight[i]
						*Bcoeff;
		       }
                    }
                 }
...
} /* end InterpolatedGradX3DTranZero */
...
{ /* begin InterpolatedGradY3DTranZero */
           ...
           if ((zIndex[j] >= 0) && (zIndex[j] < Slice)) {
              for (l = 0L; l <= SplineDegree; l++) {
				...
				+ zIndex[j] * Width * Height);
                    for (i = 0L; i <= SplineDegree; i++) {
                       if ((xIndex[i] >= 0) && (xIndex[i] < Width)) {
                          p[xIndex[i]] += zWeight[j]*gradyWeight[l]*xWeight[i]
						*Bcoeff;
                       }
                    }
                 }
...
} /* end InterpolatedGradY3DTranZero */
...
{ /* begin InterpolatedGradZ3DTranZero */
           ...
           if ((zIndex[j] >= 0) && (zIndex[j] < Slice)) {
              for (l = 0L; l <= SplineDegree; l++) {
				...
				+ zIndex[j] * Width * Height);
                    for (i = 0L; i <= SplineDegree; i++) {
                       if ((xIndex[i] >= 0) && (xIndex[i] < Width)) {
                          p[xIndex[i]] += gradzWeight[j]*yWeight[l]*xWeight[i]
						*Bcoeff;
                       }
                    }
                 }
...
} /* end InterpolatedGradZ3DTranZero */
